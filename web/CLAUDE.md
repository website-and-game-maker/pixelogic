...
A nonogram logic-puzzle game. Players deduce a hidden pixel-art picture
...
- Pure client-side; everything must work as static files under `/clueweave/`.
- `vite.config.ts` `base` is `/clueweave/` (the GitHub Pages project path).
...
npm run dev        # local dev server (http://localhost:5173/clueweave/)
...
## How to Continue
- The design spec lives at `docs/superpowers/specs/2026-06-03-clueweave-design.md`.
